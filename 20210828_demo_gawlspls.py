...
# %reset -f
"""
@author: Masaki Kosaka
(Reference: https://datachemeng.com/gawlsgavds/)
visualization added
"""
...
# load dataset
dataset = pd.read_csv('D:\\OneDrive\\デスクトップ\\Data science\\ようつべ\\実戦編\\gawls_gavds-master\\shootout_2002_train1.csv')
...
result = []
tmp = [selected_x_variable_numbers[0]]
for i in range(len(selected_x_variable_numbers)-1):
    if selected_x_variable_numbers[i+1] - selected_x_variable_numbers[i] == 1:
        tmp.append(selected_x_variable_numbers[i+1])
    else:
        if len(tmp) > 0:
            result.append(tmp)
        tmp = []
...
for j in range(len(result)):
    start_datetime = min(result[j])
    end_datetime = max(result[j])
    ax.axvspan(start_datetime, end_datetime, color="gray", alpha=0.3)

tag = np.linspace(0, x_train.shape[1], x_train.shape[1])
ax.plot(tag, x_train[1])
ax.grid()
