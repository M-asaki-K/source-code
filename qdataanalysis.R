...
library(rcdklibs)
#------------------------bp regression--------------------------
mols <- parse.smiles(as.character(complete.compounds[,c(1)]))
...
descs <- eval.desc(mols, cbind(dnames,dnames2,dnames3,dnames5), verbose=TRUE)
#topological descriptors inactivated to shorten calculation time
...
y <- complete.compounds[,c(2)] #predict EA
y
...
multi.regression.compounds <- as.data.frame(cbind(preprocessed.y, multi.regression.x))
