...
for(j in 1:(ncol(preprocessed.x) - 1)){ #the process stops when variable number becomes 1
...
for(k in 1:(ncol(preprocessed.x) - 3)){ 
  ...
  variable.importance.sum.t <- variable.importance.sum.t[-c(eliminate.t),]
