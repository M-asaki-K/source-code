...
aomori <- function(zs,ind){
   
   compounds.svr.s.t <- svm(zs[ind,-c(1)], zs[ind,c(1)],gammma = obj.s.t$best.parameters[,c(1)], cost = obj.s.t$best.parameters[,c(2)], epsilon = obj.s.t$best.parameters[,c(3)])
   abs(predict(compounds.svr.s.t, newdata = zs[ind,-c(1)]) - zs[ind,c(1)])}

kagoshima <- boot(data = zs,statistic = aomori, sim = "ordinary", stype = "i", R =10)
err <- kagoshima$t0
max(err)
...
else{zs <- zs[kagoshima$t0 < threshold,]
   i = 0
}
}
